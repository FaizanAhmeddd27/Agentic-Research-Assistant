...
export default function ResearchPage() {
  ...
  // Connect to the SSE stream for the given query and wire up all event handlers
  function connectStream(q: string) {
    if (eventSourceRef.current) {
      eventSourceRef.current.close();
    }
    setState((s) => ({ ...s, status: "streaming", query: q, error: "" }));

    const url = api.streamUrl(threadId, q);
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((r) => {
        ...
        const storedQuery = (v.query as string) || "";
        const inProgress = !draft && !final && !!storedQuery && reviewStatus === "pending";
        ...
        // An in-progress thread (sources gathered but no report yet) was interrupted —
        // reconnect to the stream so it resumes instead of hanging on "Connecting".
        if (inProgress) {
          connectStream(storedQuery);
        }
      })
      .catch(() => {
  ...
  const statusConfig: Record<string, { label: string; badge: string; dot: string }> = {
    completed: { label: "Final Report", badge: "bg-brand/10 text-brand", dot: "bg-brand" },
    awaiting_review: { label: "Awaiting Review", badge: "bg-amber-100 text-amber-900 border border-amber-300", dot: "bg-amber-500" },
    streaming: { label: "Researching", badge: "bg-brand/10 text-brand", dot: "bg-brand animate-ping" },
    idle: { label: "Idle", badge: "bg-brown/5 text-brown-lighter", dot: "bg-brown-lighter" },
    error: { label: "Error", badge: "bg-red-50 text-red-700 border border-red-200", dot: "bg-red-500" },
  };
  const st = statusConfig[state.status] || statusConfig.idle;

  return (
    <AppLayout>
      <div className="max-w-6xl mx-auto space-y-8">
        {/* Header Section */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-brown/10 pb-6 animate-fade-up">
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1.5">
              <span className="eyebrow">Research Thread</span>
              <span className="text-brown-lighter/30">•</span>
              <span className="text-[11px] text-brown-lighter font-mono">#{threadId.slice(0, 8)}</span>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-3 shrink-0">
            {state.status === "completed" && (
              <>
                <button onClick={handleCopy} className="btn btn-soft py-2.5! px-4!">
                ...
                <button onClick={handleDownloadMarkdown} className="btn btn-primary py-2.5! px-4!">
            ...
            <div className={`px-3.5 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest flex items-center gap-2 ${st.badge}`}>
        ...
        {state.error && !state.draftReport && !state.finalReport && (
          <div className="p-5 bg-red-50 rounded-2xl border border-red-200 text-sm text-red-700 flex items-center justify-between animate-fade-in">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            {state.status === "streaming" && (
              <div className="card p-6 space-y-4 border-brand/20 animate-fade-up">
            ...
            {state.nodes.length > 0 && (
              <div className="card p-6 animate-fade-up">
                <div className="flex items-center justify-between mb-5 border-b border-brown/5 pb-3">
                  <h3 className="eyebrow">Agent Execution Steps</h3>
                  <span className="text-xs text-brown-lighter font-mono">{state.nodes.length} events</span>
                </div>

                <div className="space-y-1.5">
                  {state.nodes.map((n, i) => (
                    <div key={i} className="flex items-center justify-between gap-4 p-2.5 rounded-xl hover:bg-white/50 transition-colors">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                          n.status === "done" ? "bg-brand/10 text-brand" :
                          n.status === "awaiting_review" ? "bg-amber-100 text-amber-700" : "bg-brown/5 text-brown-lighter"
                        }`}>
                          {n.status === "done" ? (
                            ...
                            <span className="w-2 h-2 rounded-full bg-brown-lighter/60" />
                          )}
                        </div>
                        <div className="min-w-0">
                          <span className="text-sm font-semibold text-brown block">{NODE_LABELS[n.node] || n.node}</span>
                          <span className="text-[11px] text-brown-lighter capitalize">{n.status.replace(/_/g, " ")}</span>
                        </div>
                      </div>
                      ...
                      <div className="text-xs text-brown-lighter text-right shrink-0">
                        ...
                        {n.node === "retriever" && <span>{String(n.source_count || 0)} sources</span>}
                        {n.node === "writer" && <span>{Math.round(Number(n.draft_length || 0) / 100) / 10}K chars</span>}
                ...
                {state.subQuestions.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-brown/5">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-brown-lighter mb-2.5">Planned Sub-Questions</h4>
            ...
            {state.status === "awaiting_review" && (state.draftReport || state.finalReport) && (
              <div className="card-white p-6 md:p-8 border-2 border-amber-300 space-y-6 animate-fade-up">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-amber-200 pb-5">
                  <div>
                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-amber-100 text-amber-900 rounded-full text-xs font-bold uppercase tracking-wider mb-2.5">
                      <span className="w-2 h-2 rounded-full bg-amber-600 animate-ping" />
                      Human-in-the-Loop Review
                    </div>
                    <h2 className="font-serif text-2xl md:text-3xl font-bold text-brown">Draft Research Report</h2>
                    <p className="text-xs text-brown-lighter mt-1.5">
                ...
                {!isEditing ? (
                  <div className="bg-cream-dark p-4 sm:p-6 md:p-8 rounded-2xl border border-brown/10 max-h-[600px] overflow-y-auto">
                ...
                {isRejecting && (
                  <div className="space-y-3 bg-red-50/50 p-6 rounded-2xl border border-red-200 animate-fade-in">
                ...
                <div className="flex flex-wrap gap-4 pt-2">
                  {!isEditing && !isRejecting ? (
                    <>
                      <button onClick={() => handleReview("approve")} disabled={submittingReview} className="btn btn-primary flex-1 min-w-[200px]">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                        </svg>
                      ...
                      <button onClick={() => { setEditText(state.draftReport || state.finalReport); setIsEditing(true); }}
                        disabled={submittingReview} className="btn btn-outline">
                        Edit Draft
                      </button>

                      <button onClick={() => setIsRejecting(true)} disabled={submittingReview} className="btn btn-ghost">
                        Request Revision
                      </button>
                    </>
                  ) : isEditing ? (
                    <>
                      <button onClick={() => handleReview("edit")} disabled={submittingReview || !editText.trim()} className="btn btn-primary flex-1 min-w-[200px]">
                        {submittingReview ? "Saving…" : "Save & Approve Edited Report"}
                      </button>
                      <button onClick={() => setIsEditing(false)} disabled={submittingReview} className="btn btn-ghost">
                        Cancel
                      </button>
                    </>
                  ) : (
                    <>
                      <button onClick={() => handleReview("reject")} disabled={submittingReview}
                        className="btn flex-1 min-w-[200px] bg-red-700 text-cream hover:bg-red-800">
                        {submittingReview ? "Sending Feedback…" : "Submit Revision Request"}
                      </button>
                      <button onClick={() => setIsRejecting(false)} disabled={submittingReview} className="btn btn-ghost">
                        Cancel
                      </button>
                    </>
                  )}
            ...
            {state.status === "completed" && (state.finalReport || state.draftReport) && (
              <div className="card-white p-5 sm:p-8 md:p-12 space-y-8 animate-fade-up">
                <div className="flex items-center justify-between border-b border-brown/10 pb-6">
                  <div>
                    <span className="eyebrow block mb-1">Synthesized & Verified</span>
            ...
            {state.status === "idle" && (
              <div className="text-center py-24 card animate-fade-in">
                ...
                <p className="text-xs text-brown-lighter mt-1.5">Preparing multi-agent graph...</p>
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="card p-6 sticky top-24">
              <div className="flex items-center justify-between mb-4 border-b border-brown/5 pb-3">
                <h3 className="eyebrow">Sources Gathered</h3>
              ...
              {state.sources.length === 0 ? (
                ...
                <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1">
                  {state.sources.map((s, i) => (
                    <div key={i} className="p-4 bg-white rounded-xl border border-brown/10 shadow-xs hover:border-brand/30 hover:shadow-md transition-all group">
                      <div className="flex items-center justify-between gap-2 mb-1.5">
                        <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider ${
                          s.origin === "rag" ? "bg-brand/10 text-brand" : "bg-blue-50 text-blue-700 border border-blue-100"
                        }`}>
                          {s.origin === "rag" ? "Docs" : "Web"}
                        </span>
                        {s.source.startsWith("http") ? (
                          <a href={s.source} target="_blank" rel="noopener noreferrer"
                            className="text-[11px] text-brand hover:underline font-mono truncate max-w-[150px] inline-flex items-center gap-1">
                          ...
                          <span className="text-[11px] text-brown-lighter font-mono truncate max-w-[150px]">{s.source}</span>
                        )}
                      </div>
